...
class DefaultElement:
    """Default constants and messages for the UI elements."""
    ...
    OPENAI_NO_CREDIT_STATUS_EN: str = "No credits"
    OPENAI_NO_CREDIT_STATUS_ES: str = "No creditos"
    OPENAI_MODEL_ACCESS_STATUS: str = "OpenAI error: model unavailable or no access."
    OPENAI_API_KEY_STATUS: str = "OpenAI error: invalid API key."
    OPENAI_GENERIC_STATUS: str = "OpenAI error: request failed."
...
class LocalChatbotUI:
    """Main UI class for the local RAG chatbot using Gradio."""
    ...
    def __init__(
        ...
        Args:
            ...
            avatar_images: Paths to avatar images for user and bot.
        """
    ...
    def _map_openai_error_status(self, error_message: str, language: str) -> str:
        ...
        if (
            ...
            if language == "eng":
                return DefaultElement.OPENAI_NO_CREDIT_STATUS_EN
            return DefaultElement.OPENAI_NO_CREDIT_STATUS_ES
        if (
            "model_not_found" in message
            or "does not exist or you do not have access" in message
        ):
            return DefaultElement.OPENAI_MODEL_ACCESS_STATUS
        if "invalid_api_key" in message or "incorrect api key" in message:
            return DefaultElement.OPENAI_API_KEY_STATUS
        return DefaultElement.OPENAI_GENERIC_STATUS
    ...
    def _get_respone(
        ...
        if self._pipeline.get_model_name() in [None, ""]:
            ...
            try:
                ...
                error_text = str(e)
                status = self._map_openai_error_status(
                    error_text, self._pipeline.get_language()
                )
                gr.Warning(status)
                yield (
                    DefaultElement.DEFAULT_MESSAGE,
    ...
    def _change_model(self, model: str):
        if model not in [None, ""]:
            try:
                ...
                gr.Error(f"Failed to change model: {str(e)}")
                return DefaultElement.MODEL_NOT_EXIST_STATUS
        return DefaultElement.DEFAULT_STATUS
    ...
    def _processing_document(
        ...
        print(f"DEBUG: Processing documents: {document}")  # Debug: Lista de documentos
        document = document or []
        print(f"DEBUG: Using documents directly: {document}")  # Debug: Usando paths directos
        self._pipeline.store_nodes(input_files=document)
        self._pipeline.set_chat_mode()
        print("DEBUG: Processing completed successfully")  # Debug: Éxito
    ...
    def build(self):
        load_dotenv()
        available_models = os.getenv("AVAILABLE_MODELS", "llama3.1:8b-instruct-q8_0,gpt-3.5-turbo,gpt-4,mistral:7b-instruct-q4_0").split(",")
