...
class OllamaSettings(BaseModel):
    """Settings for Ollama LLM models."""
...
class OpenAISettings(BaseModel):
    """Settings for OpenAI LLM models."""

    temperature: float = Field(default=0.7, description="Temperature for OpenAI models")
    max_tokens: int = Field(default=1000, description="Maximum tokens for response")
    api_key: str = Field(default="", description="OpenAI API key")


class RetrieverSettings(BaseModel):
    """Settings for the retriever component."""
...
class IngestionSettings(BaseModel):
    """Settings for document ingestion and processing."""
...
class StorageSettings(BaseModel):
    """Settings for vector storage and database."""
...
class RAGSettings(BaseModel):
    """Main settings container for the RAG chatbot."""
